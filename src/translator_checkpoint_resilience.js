...
function extractTextRunsFromBlock(blockXml, blockStartPos) {
    ...
    while ((runMatch = runRegex.exec(blockXml)) !== null) {
        ...
        // We match BOTH <w:t> tags AND <w:br/> tags (which are line breaks)
        // Note: <w:br/> is self-closing and contains no text, but represents a \n
        const nodeRegex = /(<w:t(?:\s[^>]*)?>([^<]*)<\/w:t>)|(<w:br\s*\/?>)/g;
        let nodeMatch;

        while ((nodeMatch = nodeRegex.exec(runContent)) !== null) {
            let text = '';
            let fullMatch = nodeMatch[0];

            // Check if it's a <w:t> or <w:br>
            if (nodeMatch[3]) {
                // It's a <w:br/> -> Treat as newline
                text = '\n';
            } else {
                // It's a <w:t> -> Get content
                text = nodeMatch[2];
            }

            // Keep ALL text nodes, even whitespace-only ones
            // (newlines are valuable context)
            if (text !== undefined && text !== null) {
                texts.push({
                    text: text,
                    localStart: runStart + openingTagLength + nodeMatch.index,
                    localEnd: runStart + openingTagLength + nodeMatch.index + fullMatch.length,
                    globalStart: blockStartPos + runStart + openingTagLength + nodeMatch.index,
                    globalEnd: blockStartPos + runStart + openingTagLength + nodeMatch.index + fullMatch.length,
                    fullMatch: fullMatch,
    ...
    for (const { translations, segmentMap } of allTranslations) {
        for (const [segIndex, translation] of translations) {
            ...
            if (textNodes.length === 1) {
                ...
                // CLEANUP: If we fall back, we MUST strip the tags, otherwise they appear in the doc
                ...
                const words = cleanTranslation.split(/\s+/).filter(w => w.length > 0);
